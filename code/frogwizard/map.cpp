...
namespace {
    ...
    const uint8_t grasslandMap[] FROGBOY_ROM_DATA = {
        8, 8, 8, 8, 8, 8, 8, 5, 6, 6, 6, 6, 3, 6, 7, 8, 8, 8, 8, 8, 8, 8, 5, 6, 6, 6, 6, 6, 6, 7, 8, 8, 5, 9, 10, 10, 10, 10, 11, 6, 6, 6, 7, 8, 8, 8, 8, 8, 8, 8, 8, 1, 8, 8, 2, 8, 8, 1, 8, 8, 1, 8, 2, 2, 2, 8, 1, 8, 8, 8, 8, 8, 1, 8, 8, 2, 2, 2, 8, 8, 1, 8, 8, 8, 8, 8, 8, 5, 6, 7, 8, 5, 6, 6, 7, 8, 8, 8, 8, 8,
    };
...
void Map::initSystem() {
    map.currentIndex = MAP_TYPE_GRASSLAND;
}

uint8_t Map::getPixelAttribute(int16_t x, int16_t y) const {
    int16_t mapTileWidth = static_cast<int16_t>(frogboy::readRom<uint8_t>(mapWidths + currentIndex));
    ...
    const uint8_t* mapPtr = frogboy::readRom<const uint8_t*>(mapAddresses + currentIndex);
...
bool Map::getPixelAttributeObs(uint8_t attr, int16_t x, int16_t y) const {
    x %= 8;
    y %= 8;
...
bool Map::getPixelObs(int16_t x, int16_t y) const {
    return getPixelAttributeObs(getPixelAttribute(x, y), x, y);
}

uint8_t Map::getWidth() const {
    return frogboy::readRom<uint8_t>(mapWidths + currentIndex);
}
...
void Map::draw(const Camera& camera) const {
    int16_t drawX = -((camera.x % 16 + 16) % 16);
    int16_t startColumn = camera.x / 16;
    uint8_t columnCount = frogboy::SCREEN_WIDTH / 16 + 2;
    uint8_t mapTileWidth = frogboy::readRom<uint8_t>(mapWidths + currentIndex);
    ...
    const uint8_t* mapPtr = frogboy::readRom<const uint8_t*>(mapAddresses + currentIndex) + clampedStartColumn;
    for(uint8_t col = 0; col != columnCount; ++col) {
        int16_t drawY = camera.y;
